...
# download necessary libraries (JW)
library(readr)
library(tidyverse)
library(car)
library(mice)
...
# check for missing values (JW)
sum(is.na(ontime_data))

# 2 missing - omit them (JW)
ontime_data %>% na.omit(ontime_data)

# prelim summary statistics (JW)
summary(ontime_data)
...
# check each variable for missing values (JW)
...
# produce mishandling summary statistics (JW)
...
## Have tried: KNN, mice (linear regression, ridge regression, predictive mean matching, classification and regression trees) (JW)

# First check the variance of each predictor variable - ones with variance close to 0 can be removed (JW)
variances <- apply(mishandled_data, 2, var, na.rm = TRUE)

# Display the variances - carrier and carrier_id are redundant, can be removed (JW)
variances

# remove columns unique_carrier and unique_carrier name, contain the same info as carrier and carrier_id (JW)
mishandled_data <- mishandled_data[, -c(7, 8)]

# set the imputation methods for each variable (JW)
methods <- make.method(mishandled_data)

# do not include form type in the imputation (JW)
methods["FORM_TYPE"] <- "none"

# set classification and regression trees as the method for other variables (JW)
methods[names(methods) != "FORM_TYPE"] <- "cart"

# current method: trying the mice package and classification and regression trees to impute values (JW)
imputed_data_mice <- mice(mishandled_data, method = methods, m = 1)

# Complete the dataset by filling in the missing values (JW)
mishandled_data_comp <- complete(imputed_data_mice)

# Check the first few rows of the completed data (JW)
head(mishandled_data_comp)
sum(is.na(mishandled_data_comp))

####### mishandled_baggage variable graphs pre- and post-imputation ############
# distribution of passenger variable before imputation (JW)
...
# distribution of passenger variable after imputation (JW)
...
# distribution of enplaned_baggage variable (JW)
...
# distribution of enplaned_baggage variable after imputation (JW)
...
# distribution of mishandled_wchr_sctr (JW)
...
# distribution of mishandled_wchr_sctr variable after imputation (JW)
...
# distribution of enplaned_wchr_sctr (JW)
...
# distribution of enplaned_wchr_sctr variable after imputation (JW)
...
# distribution of form_type variable after imputation (JW)
...
# distribution of mkt_carrier_flag (JW)
...
# distribution of mkt_carrier_flag variable after imputation (JW)
...
# standardize names for columns to merge on: year, month, carrier (JW)
mishandled_data %>%
  rename(Year = YEAR,
         Month = MONTH,
         Carrier = CARRIER) %>%
# display first few rows (JW)
head()

# rename the Marketing_Airline_Network column ** may revisit later (JW)
ontime_data %>% 
  rename(Carrier = Marketing_Airline_Network) %>%
  head()

# merge the two datasets based on the above columns (JW)
...
# aggregate the ontime_data dataframe based on year, month, carrier (JW)
...
# merge aggregated ontime data with mishandled data - month-level initial analysis purposes (JW)
...
# create numeric subset of merged data first (JW)
...
# make a correlation matrix of the numeric variables (JW)
...
# variables of interest: mishandled_baggage/avg_depdelay, mishandled_baggage/avg_arrdelay (JW)
linear_model <- lm(data = numeric_data_subset, formula = as.formula(paste("~ .")))

# REVISIT AFTER FILLING NA VALUES (JW)
